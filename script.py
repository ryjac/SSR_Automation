...
# Create header row
ws_results.append(["Subscriber Name", "Subscriber ID", "Birth Date", "Issue Date", "Service Date", "Primary Aid Code",
                   "Responsible County", "Trace Number", "Eligibility Message"])
...
def load_credentials():
    ...
    if username is None or password is None:
        ...
        exit(1)  # You can choose to handle the error differently, e.g., raise an exception or exit the script
    else:
...
def parse_ssr_results(response):
    ...
    subscriber_name = subscriber_name_elem.text.replace("Subscriber Name: ", "").strip() \
        if subscriber_name_elem else "Not Found"

    subscriber_id_elem = soup.find('div', {'id': 'MainContent_trssntrue'})
    subscriber_id = subscriber_id_elem.text.replace("Subscriber ID: ", "").strip() \
        if subscriber_id_elem else "Not Found"
    ...
    return [subscriber_name, subscriber_id, birth_date, issue_date, service_date, primary_aid_code, responsible_county,
            trace_number, eligibility_message]
...
def get_total_entries(ws):
    total = 0
    for row in ws.iter_rows(min_row=2, min_col=1, max_col=1):  # Assuming "recip_id" is in the first column
        if row[0].value:
            total += 1
    return total
